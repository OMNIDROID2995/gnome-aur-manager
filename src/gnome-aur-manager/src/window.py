...
class MainWindow(Gtk.ApplicationWindow):
    def __init__(self):
        ...
        self.set_icon_name("gnome-aur-manager")
        
        # Set WM_CLASS to match .desktop file for proper icon association
        # This ensures GNOME Shell properly associates the window with the app
        import ctypes
        try:
            # Get the GdkSurface from the window
            display = self.get_display()
            if display:
                # Set the program name which affects WM_CLASS
                GLib.set_prgname("gnome-aur-manager")
        except:
            pass
