...
def main():
    ...
    parser.add_argument('--infobar', action='store_true',
                        help='Force show PackageKit infobar (Debug)')
    ...
    from window import MainWindow, load_translations, _
...
def on_activate(app, MainWindowClass):
    ...
    # Auto-search for PackageKit plugin if not installed
    def check_and_search_packagekit():
        try:
            result = subprocess.run(['pacman', '-Q', 'gnome-software-packagekit-plugin-appstream-git'],
                                  capture_output=True, text=True)
            if result.returncode != 0:  # Not installed
                # Trigger search after a short delay
                GLib.timeout_add(1000, lambda: window.auto_search_packagekit() or False)
        except:
            pass
        return False
    
    GLib.idle_add(check_and_search_packagekit)
